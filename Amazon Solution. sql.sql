...
# 765 = 750 = com^749 = re^643 = can^308
select
	  p.product_id,
      p.product_name,
...
GROUP BY category_name
 ...
 select * from y ;
 */
/* 3.   = {concat two column} , (eg:fist name and last name)
...
*/
/* 4.
4.   = {lag function}, comparising above (eg : current_month vs last_month)
 Monthly Sales Trend
...
*/
/* 5.   = {not in query used}, which is not in  (eg : customer not in) [ imagin some example ]
...
/* 6.
 6.   = {join of join id / rank() over (partition by *___*  order by  *___* des/as )}
 Least-Selling Categories by State
Identify the least-selling product category for each state.
Challenge: Include the total sales for that category within each state. ---------- (need to make store procedures)----------
...
join dim_product p  on p.product_id = inv.product_id
 where stock < 10
*/
/* 9.   = {Datediff between shipping and order dates} 
...
*/
/* 12.  = { coalesce and nullif , rank } -----go to creation view----
...
*/
 ...
 /*13.  = {coalesce} , { case function used differently }
...
*/
 /*21. # For Store producers
select 
ca.category_id,
ca.category_name,
coalesce(sum(i.quantity*i.price_per_unit),100) as Total_sales
from dim_category ca
left join dim_product p on p.category_id = ca.category_id
left join order_item i on i.product_id = p.product_id
left join orders o on o.order_id = i.order_id
